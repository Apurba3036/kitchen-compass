...
const AdminDashboard = () => {
  ...
  // Stats
  const totalRevenue = bookings?.filter((b: any) => b.payment_status === "paid").reduce((sum: number, b: any) => sum + (b.batches?.courses?.price ?? 0), 0) ?? 0;
...
function CreateBatchDialog({ courses }: { courses: any[] }) {
  const [open, setOpen] = useState(false);
